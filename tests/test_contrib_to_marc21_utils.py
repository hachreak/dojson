...
def test_xslt_dump():
    ...
    with open("{}/demo_marc21_to_dc.converted.xml".format(path)) as myfile:
        expect = myfile.read().replace('\n', '')
    data = list(load('tests/demo_marc21_to_dc.xml'))
    output = dumps(data, xslt_filename='tests/demo_marc21_to_dc.xslt')
...
def test_entry_points():
    ...
    with open("{}/demo_marc21_to_dc.converted.xml".format(path)) as myfile:
        expect = myfile.read().replace('\n', '')
    data = list(load('{}/demo_marc21_to_dc.xml'.format(path)))
    output = dump(data, xslt_filename='{}/demo_marc21_to_dc.xslt'.format(path))
    assert output.decode('utf-8') == expect
